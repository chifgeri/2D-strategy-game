...
public class ModelTest
{
    ...
    {
        var weapon = new Weapon("123", 10, WeaponType.LongSword, 100);
        var attribute = weapon.GetItemAttributes();
        Assert.AreEqual("Damage", attribute.AttributeName);
        Assert.AreEqual(10.ToString(), attribute.ValueString);
    }
    ...
    {
        var armor = new Armor("123", 10, ArmorType.KnightArmor, 100);
        var attribute = armor.GetItemAttributes();
        Assert.AreEqual("Armor", attribute.AttributeName);
        Assert.AreEqual(10.ToString(), attribute.ValueString);
    }
    ...
    [Test]
    public void TestRoomStateCreation()
    {
        var data = new RoomState(new List<PlayableData>(), new List<EnemyData>(), 1, null, new List<string>());
        Assert.AreEqual(new List<PlayableData>(), data.PlayerCharacters);
        Assert.AreEqual(new List<EnemyData>(), data.EnemyCharacters);
        Assert.AreEqual(1, data.RoundNumber);
        Assert.AreEqual(null, data.CurrentId);
        Assert.AreEqual(new List<string>(), data.Order);
    }
